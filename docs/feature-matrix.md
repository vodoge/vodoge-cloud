...
| 命令 | 结果 |
| --- | --- |
| `read_logs` | ✅ 取回真实日志行，含那条 `/dev/cdc-wdm1` POLLERR——以前必须 SSH 才看得到 |
| `configure_apn` 写入 | ✅ EC200U cid 5 写入回读一致，结果里无密码 |
| `configure_apn` 只改 APN | ✅ 凭据保留（`username`/`auth`/`has_password` 未被清空） |
| 密码脱敏 | ✅ 行里有、`payload - 'password'` 读出来没有 |
| `claim_modem_candidate` | ✅ 链路全通，返回可行动的拒绝原因（该候选已升格为模组） |
| 配额判定 | ✅ `quota=1, enrolled=1` 触发超限；验后已清除 |
| eSIM 三条写命令 | ❌ **未在真卡验证**（删除不可逆，台面三张真卡不做牺牲） |
| 批准一个 `found` 候选 | ❌ 无待批准对象；伪造状态属于造假数据，没有做 |

**验证过程中发现并修掉的真 bug**：枚举 `app.command_kind` 里没有这六个新
种类，六条命令会在**入库时**被 Postgres 拒绝，一条都到不了边缘。目录的单测
只构造载荷、不入库，所以对它们不可见——只有真发一次才会暴露。迁移 0052 补上。

### 主动告警（2026-08-31）

`Alert` 从契约里的死形状变成了能用的通道：边缘节流后上报，`app.alerts` 存，
`/v1/alerts` 读，设备列表页顶部展示。

节流在**边缘**而不是云端：一个 code 开始时报一次，之后窗口内最多再报一次，
带上被压下的次数。所以表里一行 = 一次“应该被告知”，不是一次“发生”。

告警卡片放在 `/devices` 而不是落地页——落地页正是 PWA 安装截图拍的那一页，
往那里加卡片会让安装对话框展示这棵树已经不再渲染的界面，而重拍需要一个登录
好、且视口与原帧一致的浏览器。

**尚未观察到真实告警。** 云端半边是结构性验证过的（表、触发器、路由、约束
都在位），但边缘发出那一侧至今没被跑到：它盯的那个故障（`/dev/cdc-wdm1` 的
POLLERR，当天反复了几小时）在部署那次重启后就不再出现了。节流逻辑本身有六个
单测。我没有为了看到一条告警而往边缘库里塞假模组——那和伪造候选状态是同一
件事。
