...
**已定：至少一项非 probe。** 理由是账本的粒度就是「一对多项」，
`0046` 明确允许部分测量诚实地记下来（「so a partial measurement can be
recorded honestly」）。要求满分等于把「诚实记录部分结果」这件事变成惩罚。

所以四根模组在开关打开后**全部保留**。

⚠️ 注意这道闸和 `resolve()` 是**两层**，不要合并：
闸 2 管的是「这根能不能被纳管」，`resolve()` 管的是「这个操作此刻能不能做」。
香港 CSL 绑得上，但它**发短信**依然会被 `resolve()` 以
「recorded as needing a probe」拒掉——这是对的，
纳管不等于每个能力都开。
...
**已定：作废，承认它们没被测过。** 不补录——账本的全部价值在于
「里面写的都是量出来的」，把一批来源不明的断言洗成「测量」会把它降级成意见。

直接后果：**内置 TOML 要跟着删掉这 9 条**。否则一台还没收到推送的边缘机
会拿着一份含有已作废规则的内置矩阵去开闸，
而那正是「两个真相源」的经典形状。

⚠️ 删完之后内置矩阵只剩 EC20 的 3 条，而这 3 条**和在跑的那份仍然不一致**
（`data` 的有无、`sms_mt` 是 probe 还是 supported）。
这说明内置矩阵作为「第二个真相源」的问题没有被这次删除解决，
只是缩小了。彻底的解法是让它从账本派生或干脆清空，
但那超出了这次决定的范围，列在未决里。
...
##### 2026-09-05 的实测：三条发送，结论**不是**「能发」或「不能发」

发往台架上的电信卡 `+8613343135334`，闭环观察（提交结果 + 送达回执 + 收件箱）：

| 卡 | profile | 提交 | 送达回执 | 收件箱 |
|---|---|---|---|---|
| 香港 CSL `862547055142811` | Generic-International | **两次 QMI 超时** | —— | —— |
| 美国 310-240 `867018069514820` | Generic-International | `sent` | **failed，code=0x45** | 没到 |
| 移动 `867018069509705`（对照） | CN-Mobile | `sent` | **delivered，0x00** | **到了** |

🔴 **两张同 profile 的卡表现完全不同** —— 一张连提交都超时，另一张提交成功
但网络回了投递失败。所以那两次超时是**卡/网络特有**的，不是
`(EC20, Generic-International)` 这一对的属性。只凭香港那张把这一对记成
`unsupported`，就是把一张卡的问题写成了一类硬件的结论。

这正是账本要求「一对」而不是「一根」的意义，也是为什么第二张卡必须测。

**结论：这一格还是 `probe`，不动。** 已有的证据不足以支持任何一个非 probe 的
写法：说 supported 与两次超时和一次投递失败矛盾；说 unsupported 与
「提交成功」矛盾。`probe` 在这里不是偷懒，是**诚实**——它正是为这种状态留的。

还需要的是：换一张 Generic-International 的卡再测，或者查清 0x45 是什么
（可能是电信那张卡欠费停机的后果——它的收件箱里有 09-02 的欠费提醒和 09-03
的停机提醒）。⚠️ 如果是后者，那这三条测的其实是**收件方**的状态，
而不是发送方的能力。
...
**(b) 受支持硬件列表**（新建）：✅ **边缘端已完成**（2026-09-05）

`[[device]]` 段、`min_agent_version` 闸、闸 1 接上目录，全部落地并推送。

🔴 **向后兼容是设计的核心**：`DeviceGate` 有四个变体，而
`NotStated`（文档根本没有 `[[device]]` 段）和 `Absent`（列表存在、这一条不在
里面）必须分开——前者放行，后者拒。塌缩成布尔就是让「还没人写这张表」
和「写了、但没写它」产生同一个后果，而线上跑的那份矩阵**正是**没有这个段：
要是「没说」等于「不行」，新 build 上线那一刻整个机队全体过不了闸 1。

`min_agent_version` 不够格时**整份拒绝**，上一份原样留着。「装了一半」比
「没装」难查得多：云端看到 succeeded，边缘却在按一份它读不全的文档做纳管
决定——而它读不全的恰好是那道闸。

**还没做的是云端那半**：`[[device]]` 的发布方。它归 admin 站，所以卡在鉴权。

### 第 3 步：两道闸 + 追溯 + 失败安全　✅ **边缘端已完成**（2026-09-05）

纳管两道闸、追溯执行、隔离期、失败安全、面板三态显示都已落地，
**默认 `VODOGE_RETRO_ENFORCE=mark`：一行都不删，只标记并告警。**

⚠️ 第 0 步（补齐实测规则）仍未做。但顺序风险已经消掉了：
默认模式不删任何东西，而 `retro_would_unbind` 告警会在试运行期
把「如果开了会掉哪几根」逐根报上来。**先看那份名单，再开开关。**

**过程中自查出四个真 bug，全在实现里**（详见边缘仓 `7e2d140`）：

| # | 形状 |
|---|---|
| 1 | mark 模式下越线的模组**从所有告警里消失** —— 试运行期看到的是假的零，而它要度量的正是这群 |
| 2 | `gate_failure` 读失败开出一整类**零宽限删除**：一次瞬时 SQLITE_BUSY 让 Keep 不清标记，下一次真判定用 `COALESCE` 接上旧起点，30 分钟宽限变成 0 |
| 3 | 隔离和越线只记了一半标记，mark→unbind 切换时隔离期实际为零 |
| 4 | 「误删可复原」在生产代码里**不存在** —— `retired_registration` / `forget_retirement` 写了、测了、承诺了，`grep` 出来 0 次调用 |

第 1 条最严重，因为它废掉的正是这套设计里唯一的退路。

**还发现两个既有的实弹缺陷**（已修，见 `23f6d05`）：
`local_modems.family` 被退化读无条件覆盖（库里已有两行 `family = '0'`），
以及 `discoveries` 读失败时发一个捏造的空数组（云端 `0050` 早就把
「键缺席」正确地读作「别动这张表」，是边缘这半在撒谎）。
...
见下一节。⚠️ **被工具链挡住**：工作站上没有 node / gh / Go / psql / docker，
只有 Rust。admin 站的骨架已建（`vodoge-admin`，本地 git），
但建 GitHub 仓库、出产物、部署都做不了。

### 第 6 步：观察期

机队上跑几周只标记模式，看 `retro_would_unbind` 的误报率。
误报为零之后，再**逐台**打开 `VODOGE_RETRO_ENFORCE=unbind`
—— 而那一步改环境变量重启就能撤回。
...
1. **内置矩阵要不要彻底停止充当第二个真相源**——删掉作废的 9 条之后
   还剩 3 条，且和在跑的那份不一致。是让它从账本派生、清空，
   还是接受一份「首次推送之前的初始值」，没定
2. **现有各租户账本行怎么合并成一份全局账本**——它们是带 `tested_by`
