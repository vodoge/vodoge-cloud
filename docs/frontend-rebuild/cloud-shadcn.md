...
🔴 **但这些还没上线。** 2026-09-03 实测云主机（`root@43.108.53.126`）：
`vodoge-cloud-console-1` 是 **29 小时前**的构建，容器里的样式表是
`f13f5c37dea7192f.css`、**34,629 字节**——那是主题统一之前的产物。也就是说下面记的
所有东西（主题统一、Sidebar、对比度修复、密度收紧、配方内联、列表页排序筛选）
**线上一件都没有**。

这一行此前写的是「已部署到线上并验证过：样式表 43,261 字节」。那句话把**本地构建**
说成了**线上状态**，是错的——34,629 才是线上的数。部署时记得
[[no-cloud-side-builds]]：产物本地构建后上传，`up -d` 必须带 `--no-build`。
