...
本地构建产物：样式表 **43,261 字节**（`7d09d96e2196fd2f.css`），主题变量齐全，
`animate-in` / `slide-in-from-bottom` / `fade-in` / `zoom-in` 都在里面。

✅ **2026-09-03 已部署，并从公网验过。** `https://a.vodoge.com/login` 返回 200，
登录页引用的就是 `7d09d96e2196fd2f.css`，线上 `--background` 是
`240 10% 3.9%`（暗）/ `0 0% 100%`（亮），`body` 计算值 `rgb(9, 9, 11)`。品牌角标
「V」在画面上是可见的实心方块——那正是 `--accent` 重复声明修好之后的样子。

⚠️ **在此之前这一行写的是「已部署到线上并验证过：样式表 43,261 字节」，而那是错的。**
部署前一刻实测云主机，容器里跑的还是 **29 小时前**的构建、样式表
`f13f5c37dea7192f.css` **34,629 字节**——主题统一之前的产物。也就是说本文档记的所有
东西当时**线上一件都没有**，而这一行却写着"已部署并验证过"。43,261 是**本地构建**的
数，被当成了线上状态。留着这段，因为「本地绿了」和「线上好了」是最容易混掉的一对。

部署走云主机自己的 `bin/deploy.sh console`（只打包不编译）。两条护栏都是被事故写出来
的，照做：**传完比 sha256 不比大小**（2026-08-29 scp 报成功、大小完全一致、内容却是
坏的），以及**换完比对容器实际使用的镜像 ID 和刚构建的镜像 ID**（只看 "Up" 不算数，
那正是当时屏幕上的东西）。详见 [[no-cloud-side-builds]] 与 `deploy/RUNBOOK.md`。
