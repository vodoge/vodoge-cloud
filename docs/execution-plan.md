...
> 🔴 **USB 复位会把 agent 挂死(已修，别退回去)。**
> 2026-08-31：对模组做 `reset_modem_usb`（控制台提供给用户的恢复按钮）后，
> agent 完全停摆 50 分钟——进程活着、8743 在听，但轮询和面板全不动。取证：
> 主线程 `wchan = wdm_write`，持有的 fd 是 `/dev/cdc-wdm0 (deleted)`。
> **设备重新枚举后旧节点被删除，对失效 fd 的 QMI 写入在内核里永不返回**，没有
> 超时可等、没有错误可映射。主线程同时是轮询循环和无线电仲裁器，所以全挂。
> 修法在 `cdc_wdm.rs::ensure_live`：写之前查 `nlink==0`（节点被删）和
> 路径 inode 是否仍等于句柄的 inode（节点被重建）。两种都判为已断开。
> **这类故障告警报不出来**——能报的那个线程正是卡住的那个。

> 🔴 **数据面:sysfs 的 `raw_ip` 只改主机侧,模组侧要走 QMI WDA。**
> `/sys/class/net/wwanX/qmi/raw_ip=Y` 只设了主机的封帧,模组默认还是 802.3。
> 两边不一致时**双向丢包**,而所有状态命令都显示一切正常——2026-08-31 实测:
> `AT+CGACT? = 1,1`、`AT+CGPADDR` 返回和主机一致的地址、`AT+CGREG? = 0,1`、
> `AT+CSQ = 31`(满格),主机发出 2196 字节,模组计数器一字节没动。
> 必须发 WDA(服务 0x1a)SetDataFormat，即 `qmicli --wda-set-data-format=raw-ip`。
...
> 四个模组因此能同时各有默认路由,这正是"按模组选出口"的产品需要的形状。
