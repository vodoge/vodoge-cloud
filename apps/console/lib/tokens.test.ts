...
test("渠道健康的空态说的是「还没有记录」，不是「一切正常」", () => {
  ...
  assert.match(
    code,
    /lastSuccess === null[\s\S]{0,600}?labels\.neverSucceeded/,
    "「从来没成功过」和「上次成功于…」被合并成了一句",
  );

  // 🔴 而「窗口里没看到成功」又是第三种，不能和「从来没成功过」共用一句。
  //
  //    对抗复核抓到的：每条渠道只读最近 50 条。一条曾经一直成功、后来连续失败
  //    51 次的渠道，窗口里一条成功都没有 —— 第一版会把它写成「从来没成功过」，
  //    和那条真的从没通过的 webhook 一模一样。而两者的下一步相反：一个是刚刚
  //    坏掉（去看凭据），一个是配置从没对过（去看地址）。
  assert.match(
    code,
    /windowFull[\s\S]{0,200}?labels\.noSuccessInWindow/,
    "窗口满了却还说「从来没成功过」—— 窗口看不到不等于不存在",
  );
