-- 号码是从哪张卡读出来的，让云端也知道。
--
-- 0062 修好了**数据**：换卡之后旧号码不再挂在新卡名下。但它顺带制造了一个新的
-- 界面问题 —— 号码会突然变空，而空有两种意思，控制台对两种画的是同一个横杠：
--
--   问过了，这张卡就是没有号码   → 到此为止，别等了
--   换了卡，这张卡还没问出来     → 再等一轮
--
-- 运维正是靠号码认卡的，这两件事要做的正好相反。边缘端本地一直分得开（0011 建的
-- `local_modems.msisdn_iccid`），只是那个事实停在 agent 的 SQLite 里没走出去。
-- 这条迁移让它走完最后一段：上行带上，云端存下，控制台照着说话。
--
-- 🔴 上线顺序：**先边缘后云端**。老 agent 的 payload 里没有 `msisdn_iccid`，
--    这一列会是 NULL；控制台把 NULL 当作「不知道」而闭嘴，不会因此满屏假「待读」。
--
-- ⚠️ 函数体是从生产库 `pg_get_functiondef` 原样拉下来再打补丁的，和 0062 一样。
--    手抄一遍等于把这个系统最吃紧的写入路径重写一次。
BEGIN;

ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS msisdn_iccid text;
