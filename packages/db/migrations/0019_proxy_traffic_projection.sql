...
-- ProxyTraffic envelopes land in app.ingress like everything else, and like
-- DeviceState they need projecting or the traffic table stays empty however
-- much the devices report.
--
-- Folded into the hourly totals rather than replacing them: the edge reports
-- what it has counted since its last report, so a device that reconnects
-- mid-hour would otherwise erase the part of the hour it already sent.
--
-- The instance id is checked against app.proxy_instances before the row is
-- written. A device replaying an old report after its listener was deleted
-- would otherwise fail the foreign key and take the whole ingress transaction
-- down with it — losing the envelope, which is the one thing the journal
-- exists to prevent.
CREATE OR REPLACE FUNCTION app.project_proxy_traffic(
    p_tenant_id uuid,
    p_payload jsonb
)
RETURNS void
...
BEGIN
    v_hour := date_trunc(
        'hour',
        to_timestamp(COALESCE((p_payload ->> 'reported_at')::bigint, 0) / 1000.0)
    );

    FOR v_entry IN
        SELECT value FROM jsonb_array_elements(COALESCE(p_payload -> 'instances', '[]'::jsonb))
    LOOP
        CONTINUE WHEN COALESCE(v_entry ->> 'instance_id', '') = '';
        CONTINUE WHEN NOT EXISTS (
            SELECT 1 FROM app.proxy_instances AS i
             WHERE i.tenant_id = p_tenant_id
               AND i.id = (v_entry ->> 'instance_id')::uuid
        );
        ...
        VALUES (
            p_tenant_id,
            (v_entry ->> 'instance_id')::uuid,
            v_hour,
            COALESCE((v_entry ->> 'bytes_up')::bigint, 0),
            COALESCE((v_entry ->> 'bytes_down')::bigint, 0),
            COALESCE((v_entry ->> 'connections')::bigint, 0)
        )
        ON CONFLICT (tenant_id, instance_id, hour) DO UPDATE SET
            bytes_up = app.proxy_traffic.bytes_up + EXCLUDED.bytes_up,
...
ALTER FUNCTION app.project_proxy_traffic(uuid, jsonb) OWNER TO vodoge_owner;
REVOKE ALL ON FUNCTION app.project_proxy_traffic(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.project_proxy_traffic(uuid, jsonb) TO vodoge_app;

COMMIT;

-- accept_ingress gains the ProxyTraffic branch. Derived from 0014's text so
