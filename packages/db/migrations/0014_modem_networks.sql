...
-- The edge now reports what it always knew: which operator issued the card
-- (home) and which network it is registered on (serving). Both modems on the
-- bench are Hong Kong subscriptions roaming on the mainland, so without the
-- pair every card looks like a 中国联通 card — the serving network is the
-- least informative fact about it.
ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS home_plmn text;
ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS serving_plmn text;

-- Same function as 0013 with the modem projection extended; the definition is
-- derived from 0013's text rather than re-written, so everything outside the
-- modem block is character-identical.
