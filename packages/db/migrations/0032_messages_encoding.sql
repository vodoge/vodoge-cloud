-- Store the alphabet an inbound message arrived in.
--
-- The contract has carried `encoding` since v1 and app.messages had nowhere to
-- put it, so it was parsed and dropped. That was survivable while every body
-- was text. It stopped being survivable when the decoder learned to tell
-- eight-bit data apart from characters: those bodies are now hex, and without
-- this column nothing distinguishes "this message was binary" from "the
-- decoder produced garbage" -- which is exactly the confusion that hid four
-- decoding faults for weeks.
--
-- Existing rows predate the distinction and are backfilled as 'unknown'
-- rather than guessed. Some of them genuinely were mis-decoded, and claiming
-- otherwise would launder that.
--
-- Derived from 0030's text so everything outside the messages insert is
-- character-identical.
BEGIN;

ALTER TABLE app.messages ADD COLUMN IF NOT EXISTS encoding text NOT NULL DEFAULT 'unknown';

ALTER TABLE app.messages DROP CONSTRAINT IF EXISTS messages_encoding_valid;
ALTER TABLE app.messages ADD CONSTRAINT messages_encoding_valid
    CHECK (encoding IN ('gsm7', 'ucs2', '8bit', 'unknown'));
