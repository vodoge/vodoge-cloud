-- Carry the identity and host facts the edge already reads but discarded.
--
-- Four gaps, one projection, so they land together.
--
-- 1. Firmware. `app.modems.firmware` has existed since 0001 and has been NULL
--    in every row ever written, because no projection filled it. Both edge
--    identity probes read the revision to detect the modem family and then
--    dropped it, so the only way to learn which build was on a stick was to
--    issue a diagnostic command and read the answer by hand.
--
-- 2. The card's own number. Read over AT+CNUM, cached on the edge against the
--    ICCID it came from, and never carried upstream. Frequently absent for
--    real reasons -- plenty of operators never write it to the card -- so NULL
--    here means the card did not say, not that there is no number.
--
-- 3. Physical topology. The cloud cannot see the edge machine's /dev or sysfs,
--    so an operator diagnosing a module that stopped answering had to ask
--    somebody with a shell on the box which port it was even on.
--
-- 4. Host detail. CPU and memory were already carried. Disk is the one whose
--    exhaustion stops the outbox committing, and throughput and the hardware
--    identity are what the console's host card had no way to show.
--
-- Every column is nullable with no default, for the same reason 0035 gave: an
-- older agent, or a newer one whose reading failed, reports nothing rather
-- than a zero that reads like a measurement.
--
-- Derived from 0035's text so everything outside the changed projection block
-- is character-identical.
BEGIN;

ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS msisdn text;
ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS control_port text;
ALTER TABLE app.modems ADD COLUMN IF NOT EXISTS usb_device text;

ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS disk_used_bytes bigint;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS disk_total_bytes bigint;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS net_rx_bytes_per_sec bigint;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS net_tx_bytes_per_sec bigint;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS cpu_model text;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS kernel text;
ALTER TABLE app.devices ADD COLUMN IF NOT EXISTS hostname text;
        ...
        IF p_kind = 'DeviceState' THEN
            ...
            LOOP
                ...
                INSERT INTO app.modems (
                    ...
                    firmware, msisdn, control_port, usb_device
                ) VALUES (
                    p_tenant_id,
                    p_device_id,
                    v_modem ->> 'modem_imei',
