-- accept_ingress: supply messages.status on the inbound projection.
--
-- 0029 cleared the 42P10 that was killing every device session and uncovered
-- the next fault in the same statement: app.messages.status is NOT NULL with
-- no default, and this projection never named it.
--
-- Nothing caught it earlier because nothing had ever reached the statement.
-- The 0008 trigger wrote the row first, so this copy always landed on
-- ON CONFLICT DO NOTHING and its column list was never executed against a real
-- insert. Retiring the trigger promoted this to the only writer of
-- app.messages, and the omission surfaced on the very first envelope.
--
-- status became NOT NULL in 0021, when outbound messages arrived and needed a
-- send lifecycle. The trigger predates that and was never revisited.
--
-- Derived from 0028's text so everything outside the messages insert is
-- character-identical.
BEGIN;
